// Código Robô - versão 07.22


/*

        Para essa versão temos acrescentado a mudança da velocidade, que agora terá apenas 2 estados, e o controle de desaceleração
    do robô que será algo mais gradual e menos brusco.

        Como código base foi utilizado o da versão 05_17 que já possui as funcionalidades listadas abaixo funcionando:
...
uint8_t est_parada = 0;
bool flag_desacele = 0;
unsigned int cont_des = 0;
...
void IRAM_ATTR isr_temp_callback(void *args) {
  ...
  if((!status_troca) && cont_p > 10 && (!flag_desacele)){
...
void setup(){
  ...
  // --------- Configuração do sensor de Obstáculo dianteiro -----
    ...
    // Configuro para a GPIO ficar em alta impedância
     ...
     // ----- Resposta aos comando ------
        if(status_troca){               // Verifica se pode trocar (essa variável é atualizada sempre que receber um pacote)
          ...
          if(dir_t =='p'){
            ...
            // Caso não estejam permite que os motores sejam acionados
            ...
            mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_A, 20*(vel_t/1000));
            mcpwm_set_duty(MCPWM_UNIT_0, MCPWM_TIMER_0, MCPWM_OPR_B, 20*(vel_t/1000)); 
...
void f_dir(char dado){
    // Verifica qual direção/sentido foi enviado como comando
      if(dado == 'f' && !sens_obs_d){           
        ...
        if(sens_obs_d || sens_obs_t){
          mov_parado();
          flag_desacele = 0;
        }else{
          // Caso for o movimento de parada ou algum comando inválido
          cont_des = 0;
          flag_desacele = 1;
        }
      }
...
// Função para desaceleração controlada
// Reduz uma parte da velocidade a cada 100 ms (ocorre 2 vezes depois para de vez)
// Será chamada toda vez que o robô for trocar de movimento
void desaceleracao(){
